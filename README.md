...
| 变量名 | 值 |
|--------|-----|
| `QL_USERNAME` | 第一个账号的学号 |
| `QL_PASSWORD` | 第一个账号的密码 |
| `QL_SIGNIN_URL` | 签到页面完整 URL |

**多账号（可选）：**

如需签多个账号，继续添加以下变量（从 `_2` 开始依次编号）：

| 变量名 | 值 |
|--------|-----|
| `QL_USERNAME_2` | 第二个账号的学号 |
| `QL_PASSWORD_2` | 第二个账号的密码 |
| `QL_USERNAME_3` | 第三个账号的学号 |
| `QL_PASSWORD_3` | 第三个账号的密码 |

> 脚本会自动按顺序依次登录每个账号并完成签到，共用同一个浏览器进程。
